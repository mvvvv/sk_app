# Android APK Build Configuration for sk_app
# Based on StereoKit's AndroidBuild.cmake

# These are used by AndroidManifest.xml
...
# For sk_app, the glue code is already in the library (android_main provided)
# We just need to link android_native_app_glue
target_link_libraries(${APK_TARGET_NAME} PRIVATE android log)
target_include_directories(${APK_TARGET_NAME} PUBLIC ${CMAKE_ANDROID_NDK}/sources/android/native_app_glue)
target_sources(${APK_TARGET_NAME} PRIVATE
...
get_target_property(PROJECT_LIBRARIES ${APK_TARGET_NAME} LINK_LIBRARIES)
set(APK_SRC_LIBRARIES $<TARGET_FILE:${APK_TARGET_NAME}>)
set(APK_COPY_LIBRARIES lib/${CMAKE_ANDROID_ARCH_ABI}/$<TARGET_FILE_NAME:${APK_TARGET_NAME}>)
...
set(APK_BASE      ${APK_TEMP}/${APK_NAME}.1.base.apk)
set(APK_UNALIGNED ${APK_TEMP}/${APK_NAME}.2.unaligned.apk)
set(APK_UNSIGNED  ${APK_TEMP}/${APK_NAME}.3.unsigned.apk)
...
# Create dummy java file for the APK
...
add_custom_command(
	...
	COMMAND ${AAPT2} link
		...
		-I ${ANDROID_SDK_ROOT}/platforms/android-${ANDROID_MIN_SDK_VERSION}/android.jar
		${APK_TEMP}/obj/apk_resources.zip
	COMMAND cd ${APK_TEMP}/obj && ${AAPT} add ${APK_BASE} classes.dex
...
add_custom_command(
	DEPENDS
		...
		${APK_BASE}
	OUTPUT
		${CMAKE_CURRENT_BINARY_DIR}/${APK_NAME}.apk
	...
	COMMAND cd ${APK_TEMP} && ${AAPT} add ${APK_UNALIGNED} ${APK_COPY_LIBRARIES}
	COMMAND ${ZIPALIGN} -p 4 ${APK_UNALIGNED} ${APK_UNSIGNED}
	COMMAND ${APKSIGN} sign --ks ${KEYSTORE} --ks-key-alias ${KEY_ALIAS} --ks-pass pass:${KEYSTORE_PASS} --key-pass pass:${KEY_ALIAS_PASS} --out ${CMAKE_CURRENT_BINARY_DIR}/${APK_NAME}.apk ${APK_UNSIGNED}
	COMMENT "Building final APK: ${APK_NAME}.apk")

# APK build target
add_custom_target(${APK_NAME}-apk
	DEPENDS
		${APK_TARGET_NAME}
		${CMAKE_CURRENT_BINARY_DIR}/${APK_NAME}.apk
	COMMENT "Building Android APK: ${APK_NAME}.apk")

# Convenience target for installing and running the APK
add_custom_target(${APK_NAME}-run
	DEPENDS ${APK_NAME}-apk
	COMMAND ${ANDROID_SDK_ROOT}/platform-tools/adb install -r ${CMAKE_CURRENT_BINARY_DIR}/${APK_NAME}.apk
	COMMAND ${ANDROID_SDK_ROOT}/platform-tools/adb shell am start -n ${ANDROID_PACKAGE_NAME}/android.app.NativeActivity
	COMMENT "Installing and running ${APK_NAME}.apk")
